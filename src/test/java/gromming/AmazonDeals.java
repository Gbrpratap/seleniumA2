...
public class AmazonDeals {
	...
	public class FindElementsProductionTest {
		...
		public void searchMobile() {
			...
			String targetBrand = prop.getProperty("target_brand");
			String expectedCategory = prop.getProperty("expected_category");
			...
			//Filter the mobiles
			String bubbleCategoryXpath = String.format("//button[text()='%s']", expectedCategory );
			...
			for(int i=0; i<scanLimit; i++) {
				...
				Assert.assertTrue(visibleText.contains(targetBrand), "Assertion Failed: Detected a leaked/non-matching item at position " + (i+1)+ " . Text found: " + visibleText);
			}
